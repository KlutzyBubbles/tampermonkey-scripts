...
(function () {
    ...
    const makeRateUpdater = (row, decimals) => (when) => {
        if (when === 5) {
            row.fields.each(function (i) { this.innerText = formatNumber(timeframes[i] * row.rates[i], decimals[i]); });
            row.fields.gbYear.innerText = formatNumber(row.values.gbYear * row.rates[4] / row.rates[4], decimals[5]);
        } else {
    ...
    const updaters = [
        ...
        .flatMap(({ tbl, decimals }) => parseTable(tbl).map(p => ({ tbl: p, decimals })))
        .map(({ tbl, decimals }) => makeRateUpdater(tbl, decimals));
